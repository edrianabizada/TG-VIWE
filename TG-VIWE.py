...
S = '\033[1;33m'
logo=(f'''{B}{E}===================================================>
{E} THIS SCRIPT FREE FOR ALL PLEASE  DON'T SELL AND SCAM ANY ONE...{B}
|         {G}[+] DEVELOPER : {B}EDRIS NABIZADA             |
|         {G}[+] GITHUB USERNAME  : {B}EDRISNABIZADA       |
|         {G}[+] TELEGRAM USERNAME : {B}@EDRISNABIZADA420  |
|         {G}[+] TELEGRAM CHANNEL  : {B}@BEST_HACKER00420  |
{E}=====================================================''')
def tui():
    while True:
        print(logo)
...
print(logo)
channel, post = input(f"\n{G}[+] TELEGRAM VIEW POST LINK ==> {S}").replace('https://t.me/', '').split('/')
...
try:
    ...
    print(f'{G}[√]STARTED | WAIT FEW SECONDS TO RUN THREADS')
